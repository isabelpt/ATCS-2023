"""
@author: Isabel Prado-Tucker
@version: May 2024

Comments partially generated by ChatGPT
Song analysis builds off this tutorial: https://librosa.org/doc/latest/tutorial.html 
"""
import librosa
import pygame
import numpy as np
from PIL import Image, ImageOps, ImageDraw
import colorgram

# Controls the song and image analysis, and generates/draws the rotated record images
class Song:
    def __init__(self, song_name, width, height):
        """
        Initializes the Song object foe song_name with screen width and height,

        Parameters:
            song_name (String): Song chosen by user
            width (int): screen width
            height (int): screen height
        """
        # Load audio data and album cover
        self.song_name = song_name
        self.song_fp = "_assets/song" + str(self.song_name) + "/song.mp3"
        self.cover_fp = "_assets/song" + str(self.song_name) + "/cover.png"
        self.y, self.sr = librosa.load(self.song_fp)
        self.album_cover = pygame.image.load(self.cover_fp)
        self.album_cover = pygame.transform.scale(self.album_cover, (width, height))

        # Calculate spectrogram and related data
        self.stft = np.abs(librosa.stft(self.y, hop_length=512, n_fft=2048*4))
        self.spectrogram = librosa.amplitude_to_db(self.stft, ref=np.max)  
        self.frequencies = librosa.core.fft_frequencies(n_fft=2048*4)  
        self.times = librosa.core.frames_to_time(np.arange(self.spectrogram.shape[1]), sr=self.sr, hop_length=512, n_fft=2048*4)

        # Calculate ratios for indexing
        self.time_index_ratio = len(self.times)/self.times[len(self.times) - 1]
        self.frequencies_index_ratio = len(self.frequencies)/self.frequencies[len(self.frequencies)-1]

        # Extract top colors from the album cover image
        self.top_colors = self.get_top_colors(self.cover_fp)
        # Create and load record image
        self.record_img = self.create_record(self.cover_fp, "_assets/img/record_img.png")
        self.record = pygame.image.load("_assets/img/record_img.png")
        # Generate rotated images of the record for the spinning effect
        self.rot_images = []
        self.make_rotated_images(30)
        self.counter = 1 # Tracks rotation

    def get_tempo(self):
        """
        Gets the estimated tempo of the song with librosa 

        Returns:
            int: An int representing the bpm of the track
        """
        tempo, beat_frames = librosa.beat.beat_track(y=self.y, sr=self.sr)
        return tempo
    
    def get_decibel(self, target_time, freq):
        """
        Retrieve decibel value at a given time and frequency

        Returns:
            float: A float representing the decibel at a certain frequency
        """
        return self.spectrogram[int(freq * self.frequencies_index_ratio)][int(target_time * self.time_index_ratio)]
    
    '''
    Start of section written by me for previous ATCS project (Platform Game) and adapted for new purpose
    '''
    # Extract top colors from the image
    def get_top_colors(self, image_url):
        # Open the image
        image = Image.open(image_url)

        # Extract colors from the image
        colors = colorgram.extract(image, 10)

        # Get the RGB values of the top 5 colors
        top_colors = [(color.rgb.r, color.rgb.g, color.rgb.b) for color in colors]
        return top_colors

    # Create a circular mask to crop the record image
    def create_record_mask(self, size):
        # Create a new image with a white background
        mask = Image.new('L', (size, size), 255)

        # Create a circular mask with a transparent center
        draw = ImageDraw.Draw(mask)
        center = (size // 2, size // 2)
        radius = size // 2 - 5

        # Draw a filled circle at the center of the mask
        draw.ellipse((center[0] - radius, center[1] - radius, center[0] + radius, center[1] + radius), fill=0) # Outer circle
        draw.ellipse((center[0] - 110, center[1] - 110, center[0] + 110, center[1] + 110), fill=255) # Inner circle cutout
        mask = Image.eval(mask, lambda px: 255 - px)
        return mask

    # Create colored circle for the center of the record
    # TODO: Add inner silver circle
    def add_circle(self, record, position, radius, color):
        # Create a transparent overlay
        overlay = Image.new("RGBA", record.size, (0, 0, 0, 0))

        # Draw a circle on the overlay
        draw = ImageDraw.Draw(overlay)
        draw.ellipse((position[0] - radius, position[1] - radius, position[0] + radius, position[1] + radius), fill=color)
        draw.ellipse((position[0] - 15, position[1] - 15, position[0] + 15, position[1] + 15), fill=(150, 150, 150))


        # Composite the overlay and the original image
        record = record.convert('RGBA')
        image_with_overlay = Image.alpha_composite(record, overlay)

        # Save the resulting image
        return image_with_overlay
    
    # Create full record image
    def create_record(self, album_cover_url, out_path):
        # Open the base and overlay images
        base_img = Image.open(album_cover_url)
        overlay_img = Image.open('_assets/img/record.png')
        overlay_img = overlay_img.crop((25,25,487,487))
        overlay_img = overlay_img.convert('RGBA').resize(base_img.size)

        # Define the position where the overlay image will be pasted
        position = (0, 0)

        # Overlay the image over base image
        base_img.paste(overlay_img, position, overlay_img)
        mask_size = min(base_img.size)
        mask = self.create_record_mask(mask_size)
        base_img.putalpha(mask)
        base_img = self.add_circle(base_img, (base_img.width/2, base_img.width/2), base_img.width/5.75, self.top_colors[0]) # Issue with radius size

        # Save the resulting image
        base_img.save(out_path)
    '''
    End of section written by me for a previous ATCS project (Platform Game) and adapted for new purpose
    '''
    
    def make_rotated_images(self, num):
        """
        Generates and saves num rotated images of the record

        Parameters:
            int: The number of images to create
        """
        orig_img = Image.open("_assets/img/record_img.png")
        inc = 360/num
        for n in range(1, num):
            new_img = orig_img.rotate(n*inc)
            new_img.save("_assets/img/record" + str(n) + ".png")
            self.rot_images.append(pygame.image.load("_assets/img/record" + str(n) + ".png"))
    
    def draw(self, screen, radius, loc):
        """
        Draw the record image on the screen

        Parameters:
            screen (Pygame Screen): Scren to draw visual onto.
            radius (int): Radius of the record image, changes with the beat
            loc (tuple): Location of the center of the record
        """
        # Reserts to first image if the full rotation has been completed
        if self.counter >= len(self.rot_images):
            self.counter = 1

        # Draws correct rotated image for the given iteration and scales to current beat size
        img = pygame.transform.scale(self.rot_images[self.counter], (2*radius, 2*radius))
        screen.blit(img, loc)
        self.counter += 1